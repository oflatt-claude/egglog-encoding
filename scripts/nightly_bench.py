...
That cache is the published ``index.jsonl``, so the nightly server serves the
run's rows next to the page they render. Each run builds in an empty directory:
report JSONL is a disposable cache with no migrations, so rows an earlier run
wrote under another schema version would fail every ``bench.py`` invocation
rather than be recomputed.

The last run writes the page. The finished directory replaces
``nightly/output/`` only after that run succeeds, so a failed run leaves the
previously published page in place.
...
def _staging_dir(output_dir: Path) -> Path:
    """Return an empty directory beside ``output_dir`` to build this run in."""
...
def _publish(staging_dir: Path, output_dir: Path) -> None:
    """Replace the published directory with the finished run."""
...
def main(argv: Sequence[str] | None = None) -> int:
    """Populate this run's cache and publish it as ``<output_dir>/index.jsonl`` beside the page."""
    ...
    if not rendered or not (staging_dir / PAGE_NAME).is_file():
        print("nightly: benchmark did not produce a report", file=sys.stderr)
        return 1
    _publish(staging_dir, output_dir)
