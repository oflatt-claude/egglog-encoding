...
;;; slot.  The generated binder rule here removes the bound slot from the whole
;;; node, so the class comes out with no slots at all.  The two are then not the
;;; same language on this term, and `xarray.py extra` sees the difference as a
;;; BASELINE disagreement -- before any rule runs.
;;;
;;; Recorded rather than fixed: expressing it needs `:binder` to name the columns
;;; a binder covers, not just the column its slot sits in.  Nothing the 8 rules
;;; build has this shape, because a pattern's `?e` never carries the pattern slot
;;; the same pattern binds.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(push)
...
;; the reference says this class has one slot; here it has none
(check (= (ClassSlots $L) (map-empty)))
(check (RenamesToLeader $L m $L) (= (map-length m) 0))

;; and so the two applications below, which the reference keeps apart, are one
;; class here
(let $a0 (App2 "app" (map-empty) $L (map-of 0 0) (Var 0)))
(let $a1 (App2 "app" (map-empty) $L (map-of 0 1) (Var 0)))
(run-schedule (saturate (run slotted)))
(check (RenamesToLeader $a0 m1 l) (RenamesToLeader $a1 m2 l))
