...
impl ProofStore {
    ...
    fn compute_rule_substitution(
        ...
        for (fact, proof_id) in rule.body.iter().zip(premise_proofs.iter()) {
            // A side condition carries only an `Eval` marker, so its output is
            // recomputed here (the same evaluation `check_side_condition` makes)
            // rather than read off a premise. Facts come in body order, so the
            // primitive's inputs are already bound.
            if crate::proofs::proof_checker::is_side_condition(fact) {
                self.bind_side_condition(fact, &mut current_subst);
                continue;
            }
    ...
    /// Bind the variable a side condition determines, by evaluating the
    /// primitive on the bindings so far. A fact that binds nothing, or whose
    /// inputs are not all bound, contributes nothing — the checker reports it.
    fn bind_side_condition(&mut self, fact: &ResolvedFact, subst: &mut IndexMap<String, TermId>) {
        let ResolvedFact::Eq(_, lhs, rhs) = fact else {
            return;
        };
        let (var, call) = match (lhs, rhs) {
            (ResolvedExpr::Var(_, var), call) if !subst.contains_key(&var.name) => (var, call),
            (call, ResolvedExpr::Var(_, var)) if !subst.contains_key(&var.name) => (var, call),
            _ => return,
        };
        let bound: HashMap<String, TermId> = subst
            .iter()
            .map(|(name, term)| (name.clone(), *term))
            .collect();
        if let Ok(term) = self.eval_expr_with_subst("side condition", call, &bound) {
            self.add_to_subst(subst, &var.name, term);
        }
    }
