...
//! ## Ergonomic sugar
//!
//! The object-safe trait methods take erased closures (`&mut dyn FnMut(..)`) and
//! carry a `_dyn` suffix. Generic, ergonomic wrappers ([`Backend::for_each`],
//! [`Backend::with_execution_state`], [`Backend::base_value_constant`], …) are
//! provided as inherent methods on `dyn Backend` so call sites read naturally.
//!
...
//! `impl Backend for egglog_bridge::EGraph` (see `backend_impl`), leaving the
//! bridge crate itself untouched.
...
pub use egglog_bridge::{
    ActionRegistry, AtomId, ColumnTy, DefaultVal, FunctionConfig, FunctionId, MergeFn, QueryEntry,
    RuleId, ScanEntry, Variable,
};
...
/// (this crate's `backend_impl`), and out-of-tree engines such as the FlowLog
/// (differential-dataflow) backend in egglog-experimental.
///
...
pub trait Backend: Send + Sync {
    ...
    /// [`Backend::for_each`] sugar on `dyn Backend`.
    ...
    /// primitives). Object-safe form; prefer [`Backend::with_execution_state`].
    ...
    /// [`Backend::with_execution_state_tracked`].
    ...
    /// Whether this backend supports the `subsume` action / `is_subsumed`
    /// filter.
...
/// `egglog_bridge::RuleBuilder`: the reference impl is a thin passthrough; an
/// out-of-tree backend accumulates these calls into its own IR and finalizes on
/// [`RuleBuilderOps::build`].
pub trait RuleBuilderOps {
    ...
    // -- optional: disjunctive (`or`) patterns ------------------------------

    /// Like [`RuleBuilderOps::query_table`], but returns the atom's [`AtomId`]
    /// so it can be grouped into an `or` union branch via
    /// [`RuleBuilderOps::set_union_branches`]. Only backends that support
    /// disjunctive query patterns implement this.
    fn query_table_branch(
        &mut self,
        _func: FunctionId,
        _entries: &[QueryEntry],
        ...
        Err(anyhow::anyhow!(
            "disjunctive (`or`) query patterns are not supported on this backend"
        ))
    }

    /// Fuse the given branches (each a set of body atoms identified by
    /// [`AtomId`]) into a disjunctive union over the shared `output_vars`.
    /// Only backends that support `or` patterns implement this.
    fn set_union_branches(&mut self, _branch_atoms: Vec<Vec<AtomId>>, _output_vars: Vec<Variable>) {
        unimplemented!("disjunctive (`or`) query patterns are not supported on this backend")
    }
