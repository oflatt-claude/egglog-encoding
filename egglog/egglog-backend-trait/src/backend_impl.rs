...
use crate::{
    AtomId, Backend, BaseValues, ColumnTy, ContainerValues, ExecutionState, ExternalFunction,
    ExternalFunctionId, FunctionConfig, FunctionId, IterationReport, PanicMsg, QueryEntry,
    ReportLevel, RuleBuilderOps, RuleId, ScanEntry, Value, Variable,
};
...
impl RuleBuilderOps for BridgeRuleBuilderOps<'_> {
    ...
    fn query_table_branch(
        ...
        self.inner.query_table(func, entries, is_subsumed)
    }

    fn set_union_branches(&mut self, branch_atoms: Vec<Vec<AtomId>>, output_vars: Vec<Variable>) {
        self.inner.set_union_branches(branch_atoms, output_vars);
    }
