...
- Add the slotted matching frame: base sorts `Frame`, `Binding` and `Names`, and the primitives `root`, `child`, `lit`, `bound`, `leaf`, `bugs`, `atom`, `frame-join`, `anchor`, `refine`, `mint`, `free`, `not-free`, `same`, `bool-same`, `ren`, `node-slots` and `without`. A frame is the closure of the equations a slotted pattern's atoms place on slot occurrences, under two cliques that never collapse: the slots of one e-node or class, and the distinct literals. `atom` builds one atom's, `frame-join` unions two (associative and commutative), `anchor` spells the frame in a variable's slot names, `refine` enumerates the consistent mergings of what the pattern left open, and `ren` reads a variable's renaming out. `find-mapping-unify` and `refine-namings` are removed; `find-mapping`, `find-mapping-total` and `find-mappings-total` stay.
