...
- `(fail <command>+)` now accepts multiple commands, running them in order and succeeding if any one fails (previously it wrapped a single command). Desugaring keeps the whole expansion of each allowed command inside the `fail`. Persistent definitions and user-defined commands are rejected inside `fail` in every mode. Under proof encoding, each source action expands to one local action block, and potential actions keep stable proof indices through the wrapper, so successful mutations before the swallowed error remain provable.
