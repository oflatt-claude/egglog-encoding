...
- In the term/proof encoding, run each top-level action's minted temporaries as one immediate local-scope action block (a new internal `Actions` command run through `eval_actions`) instead of separate top-level `let`s. A minted `let` no longer becomes its own global function/table, so building a large static graph under the encoding no longer creates a table per proof node (the dominant cost on graphs with many top-level terms). A per-block subterm memo preserves structural dedup within a block, which now runs with no intermediate merge.
