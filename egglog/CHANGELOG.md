...
- **New proof rule: projection.** `Justification::Proj` derives `ci = ci` from a proof of `t = f(..., ci, ...)`, so a proof about a term is also a proof about each of its arguments. It is stated at the equality level rather than as a separate "is built" judgement, since in egglog a proof of `t = t` is exactly that judgement. Proofs the encoder produces are unchanged — it declares the `@Proj` proof-node relation but writes none, since it already holds every reflexive anchor it needs; the rule is there so a proof handed to the checker may use it.
