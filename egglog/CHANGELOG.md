...
- **The term/proof encoding no longer keeps a per-sort table of reflexive proofs.** `@<Sort>Proof`, which held a `t = t` proof for every term and every container, is gone: each reflexive anchor is now projected out of a row proof already in scope — `Proj` for a term a view row mentions as a child, `Trans`/`Sym` for a constructor view's e-class, and a new raw-only `ProjAll` (`CongrAll`'s projection counterpart, desugared to `Proj` during proof conversion) for a container element, whose position in the term form is only known once the term is in hand. A value a body primitive reads out of a container has no anchor at all and becomes a side condition the checker re-evaluates, alongside the container-producing primitives that already were; re-evaluation now also binds the variable such a fact determines, so a rule head may use it. Encoding every `egglog/tests` program in proof mode goes from 77,101 statements to 58,493 (-24%), and the top-level block for `(Add (Num 1) (Num 2))` from 25 statements to 19. Rows per firing: a flat rewrite 2 -> 1, the nested head in `proof_encoding.md` 6 -> 3, and a view rebuild 1 -> one packed row plus one anchor per canonicalized column that did not move. `web-demo/math`, `web-demo/herbie` and `web-demo/typeinfer` run in 0.90x, 0.89x and 0.92x their previous wall time. A sort's `:internal-ast-func` annotation now carries its `@Ast<Sort>` wrapper rather than the deleted proof table.
