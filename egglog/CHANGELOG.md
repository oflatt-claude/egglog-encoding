...
- Add the slotted matching frame: base sorts `Frame`, `Binding` and `Names`, and the primitives `root`, `child`, `lit`, `bound`, `leaf`, `bugs`, `atom`, `frame-join`, `anchor`, `refinements`, `mint`, `free`, `not-free`, `same`, `bool-same`, `ren`, `node-slots` and `without`. A frame is the closure of the equations a slotted pattern's atoms place on slot occurrences, under three cliques, sets of occurrences that must stay distinct slots: the slots of one e-node, the slots of one class, and the different literals. `atom` builds one atom's, `frame-join` unions two (associative and commutative), `anchor` spells the frame in a variable's slot names, `refinements`, registered on a `Vec` of frames, returns every consistent merging of what the pattern left open for `vec-get` to read, and `ren` reads a variable's renaming out.

- Add `all_enodes` on `Read`: every e-node of every constructor, each table scanned once, with `Enode::name` and `Enode::eclass` saying where each row came from. `eclass_enodes` answers one class by scanning every table, so reading the e-nodes of many classes means one scan per class; a caller that walks a term buckets `all_enodes` by e-class instead.

- Add `shape` on a `Vec` of renamings: a node's edges with their slots renumbered `0, 1, 2…` by first occurrence, followed by the renaming from those numbers back to the node's own names, so that nodes equal up to a renaming of their slots have equal shapes.
