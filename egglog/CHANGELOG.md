...
- In the term/proof encoding, rebuild a view's eq-sort children with a single `UF`-driven rule instead of a per-column fan-out. Each view gets one hidden `:merge` term→row index per distinct child eq-sort, and one rule — a `UF` edge joined against that index — re-canonicalizes the whole row with a `uf_canon` primitive, mirroring how a native rebuild iterates the e-nodes referencing a changed e-class rather than matching the view once per column. `uf_canon` is the generic view-column read over the two-output `UF_<S>` table, so every backend services it against its own storage.
