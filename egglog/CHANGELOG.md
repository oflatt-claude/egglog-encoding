...
- `(fail <command>+)` now accepts multiple commands, running them in order and succeeding if any one fails (previously it wrapped a single command). Desugaring keeps the whole expansion of each allowed command inside the `fail`. Persistent definitions and user-defined commands are rejected inside `fail` in every mode; proof mode also rejects operations whose proof-bearing state could outlive a swallowed error.
