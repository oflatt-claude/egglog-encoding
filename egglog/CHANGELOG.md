...
- `(fail <command>+)` now accepts multiple commands, running them in order and succeeding if any one fails (previously it wrapped a single command). Desugaring keeps the whole expansion of each allowed command inside the `fail`. `prove`, `prove-exists`, persistent definitions, user-defined commands, and explicit `push`/`pop` are rejected inside `fail` in every mode. General `fail` execution is transactional for e-graph state: it restores a snapshot whether a wrapped command fails or they all succeed. The common single-`check` form skips the snapshot because it is read-only. Under proof encoding, each source action still expands to one local action block inside the wrapper, and the rollback ensures no transient proof state escapes. Proof mode does not support `output`, either at top level or inside `fail`.
