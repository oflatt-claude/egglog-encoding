...
- A printed rule proof's `(substitution …)` list is now ordered by where each variable first occurs in the rule body, instead of by hash order over the variable names. The names are generated symbols, so any change to how many proof nodes are minted used to reshuffle the whole list; the bindings themselves are unchanged.
- In proof mode, matching a custom function (`(= v (f x y))`) no longer composes a congruence step onto the row proof for arguments that are plain variables or constants. Such a step rewrites a child by its own reflexive term proof, so the proof simplifier discarded it, but the rule still read the argument's `<S>Proof` and minted the node on every firing. The constructor arm already skipped variables and constants; the custom-function arm now does too. Proof output is unchanged.
- In proof mode, that rebuild composes each column's congruence step through a guarded backend op (`Backend::register_guarded_mint`), which mints a chain of relation rows only when a guard row exists and otherwise returns its accumulator untouched. A column that did not move now costs one `@UF` read and mints nothing, where the rule previously read the column's reflexive term proof and its `@UF` proof column and minted a `Congr` — or, for the e-class, a `Sym`/`Trans` pair — that the proof simplifier then discarded. The Differential Dataflow backend services the op against its host-side mirror.
