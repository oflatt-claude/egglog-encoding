...
- Add a common-subexpression prepass to the term/proof encoding: within each action scope (rule head, top-level action, merge body), a constructor application that occurs more than once is bound to a shared `let` and its occurrences rewritten to that variable, so the encoding interns it once instead of per occurrence (`(Foo (Bar x) (Bar x))` builds `(Bar x)` once). The pass is syntactic and runs before lowering, which already builds a `let`-bound term once and reuses its e-class — and, in proof mode, its natural node and connector — across references. Substantially reduces proof-mode time and memory on programs with heavily shared subterms (e.g. compiled model graphs).
