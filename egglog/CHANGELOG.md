...
- Add the slotted matching frame: base sorts `Frame`, `Binding` and `Names`, and the primitives `root`, `child`, `lit`, `bound`, `leaf`, `bugs`, `atom`, `frame-join`, `anchor`, `refine`, `mint`, `free`, `not-free`, `same`, `bool-same`, `ren`, `node-slots` and `without`. A frame is the closure of the equations a slotted pattern's atoms place on slot occurrences, under three cliques, sets of occurrences that must stay distinct slots: the slots of one e-node, the slots of one class, and the different literals. `atom` builds one atom's, `frame-join` unions two (associative and commutative), `anchor` spells the frame in a variable's slot names, `refine` enumerates the consistent mergings of what the pattern left open, and `ren` reads a variable's renaming out.
