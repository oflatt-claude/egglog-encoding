...
- **A view rebuild anchors only the columns that did not move.** The reflexive anchor a rebuild falls back on when a column turns out to be canonical is minted by `@UF_<Sort>_canon_proof` itself, on the miss that says so, rather than by the rule that calls it — a column that moved reads the `@UF` row's proof and writes no anchor row at all. Encoding every `egglog/tests` program in proof mode goes from 58,493 statements to 56,263. Proofs are unchanged: the projection position -1, which the fallback uses for a constructor view's e-class, reads back as the reflexivization `Trans(p, Sym(p))` the rebuild spelled out before. Backends service the new op through `Backend::register_view_column_read_or_mint`.
- **New proof rule: projection.** `Justification::Proj` derives `ci = ci` from a proof of `t = f(..., ci, ...)`, so a proof about a term is also a proof about each of its arguments. It is stated at the equality level rather than as a separate "is built" judgement, since in egglog a proof of `t = t` is exactly that judgement.
- **The term/proof encoding no longer keeps a per-sort table of reflexive proofs.** `@<Sort>Proof`, which held a `t = t` proof for every term and every container, is gone: each reflexive anchor is now projected out of a row proof already in scope — `Proj` for a term a view row mentions as a child, `Trans`/`Sym` for a constructor view's e-class, and a new raw-only `ProjAll` (`CongrAll`'s projection counterpart, desugared to `Proj` during proof conversion) for a container element, whose position in the term form is only known once the term is in hand. A value a body primitive reads out of a container has no anchor at all and becomes a side condition the checker re-evaluates, alongside the container-producing primitives that already were; so does a fact equating two such values, like `(= (vec-get v 0) (vec-get v 1))`. Re-evaluation now also binds the variable such a fact determines, so a rule head may use it. Encoding every `egglog/tests` program in proof mode goes from 77,101 statements to 58,493 (-24%), and the top-level block for `(Add (Num 1) (Num 2))` from 25 statements to 19. Rows per firing: a flat rewrite 2 -> 1, the nested head in `proof_encoding.md` 6 -> 3, and a view rebuild 1 -> one packed row plus one anchor per canonicalized column that did not move. `web-demo/math`, `web-demo/herbie` and `web-demo/typeinfer` run in 0.90x, 0.89x and 0.92x their previous wall time. A sort's `:internal-ast-func` annotation now carries its `@Ast<Sort>` wrapper rather than the deleted proof table.
...
- **The term/proof encoding emits a much smaller program.** A term/proof/AST node is written by one `mint-<Relation>!` call — minting the id and asserting the row together — instead of a `get-fresh!` binding plus a `set`, and a reflexive `Fiat` names one `@Ast` node rather than two equal ones. Encoding every `egglog/tests` program in proof mode goes from 199,408 statements to 153,100 (-23%); the top-level block for `(Add (Num 1) (Num 2))` goes from 44 statements to 25, and from 6 `@Ast` rows to 3. Rows written per rule firing and the user-facing proof text are unchanged. Backends service the new op through `Backend::register_mint_row`.
