...
- **Fix rule-set time going unrecorded when a user-defined command runs the rules.** The overall run report was written by the `run-schedule` command, so rule sets driven straight through `EGraph::step_rules` — which egglog-experimental's scheduler does for every `(run <ruleset>)` step — reached no report at all, hiding 10 of 16 rule sets on `luminal-llama`. Recording now happens in `step_rules` and `step_rules_with_scheduler`. Both are public API: an embedder that already unions their returned report into its own will now count those iterations twice.
- **The benchmark timing summary accounts for the work outside rule-set execution.** Schema version 3 adds each rule set's assembly time and the phases that run outside every rule set (parse, typecheck, desugar, encode, declare-and-compile, top-level actions, schedule, proof extraction), so the residual is what nothing measured rather than everything nothing measured. It is 1–5% of wall on `luminal-llama` and `eggcc-2mm-pass1`, native and proof mode.
- Fix a global's shared table outliving, or failing to outlive, the command that declared it: a declaration left inside a `(fail …)` that short-circuited before reaching it, and a command rejected after its globals were name-checked keeping both its rows and its names.
- Fix a desugared program failing to replay under the term/proof encoding. `:internal-global-table` is not surface syntax, so a replayed shared table arrives as a plain `:internal-let` function; the encoder now recognizes either spelling.
- Fix a `:merge` body that reads a global: the read was encoded as building a term, and the proof checker re-ran the body with only `old`/`new` bound. Also `(get-size! "<name>")` now skips reserved tables as the no-argument form did, and `(print-function <global> n)` respects `n`.
- **A global bound to a primitive works under the term/proof encoding**, where it was rejected outright. A global is now asked whether its value is, or contains, an e-class: one that does keeps the constructor-like encoding, one that does not takes the ordinary `:no-merge` route and is justified by its value — the same proof the fact would have with the value written literally. A global bound to a *container* is still rejected, and says why.
- **Every global of a sort shares one table.** Each global was its own nullary function, so the term/proof encoding gave each one a term relation, an FD view, a rebuild index and a rebuild rule; 1000 global `let`s expanded to 9049 encoded commands against 53 for 1000 local ones. A global is now one row of `@Globals_<Sort>`, keyed by a slot id and named by the fresh generator, so that schema is emitted once per sort and no program can declare a colliding function. **Because that name is reserved, a `let` no longer counts towards `(get-size!)` in either form** — matching `print-size`, which already skipped globals. On `luminal-llama` this is 8.07 s to 6.38 s under `--proofs` and 4.47 s to 3.19 s under `--term-encoding`; a program binding few globals pays the read side without the win, leaving `eggcc-2mm-pass1` about 1% slower. Proof output is unchanged.
