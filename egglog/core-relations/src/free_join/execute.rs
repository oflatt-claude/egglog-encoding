...
enum DynamicIndex {
    ...
    /// An occurrence index: one key per value appearing in *any* of the covered
    /// columns. Unlike [`DynamicIndex::DynamicColumn`] a row can be reached under
    /// several keys, so this stays a hash-keyed [`ColumnIndex`] rather than a
    /// sorted array over one column.
    DynamicOccurrence(Arc<ColumnIndex>),
...
impl Prober {
    fn get_subset<'a>(&'a self, key: &'a [Value]) -> Option<PotentiallyStale<SubsetRef<'a>>> {
        match &self.ix {
            ...
            DynamicIndex::DynamicOccurrence(tab) => {
                debug_assert_eq!(key.len(), 1);
                tab.get_subset(&key[0]).map(PotentiallyStale::not_stale)
            }
    ...
    fn for_each(&self, mut f: impl FnMut(&[Value], PotentiallyStale<SubsetRef>)) {
        match &self.ix {
            ...
            DynamicIndex::DynamicOccurrence(tab) => tab.for_each(|k, v| {
                f(&[*k], PotentiallyStale::not_stale(v));
            }),
            DynamicIndex::SparseColumn(tab) => {
                tab.for_each(|k, v| f(k, PotentiallyStale::not_stale(v)));
    ...
    fn len(&self) -> usize {
        match &self.ix {
            ...
            DynamicIndex::DynamicOccurrence(tab) => tab.len(),
