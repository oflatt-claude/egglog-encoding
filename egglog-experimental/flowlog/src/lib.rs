...
use compile::{row_col, unpack_row, BodyOp, HeadOp, MergeMode, Row, RuleIr, Slot};
...
/// Map a single-output-column [`MergeFn`] to the FlowLog [`MergeMode`] that
/// resolves that column's FD conflict:
///   - `AssertEq` / `Old`     => keep the old value
///   - `New`                  => keep the new value
///   - `UnionId`              => lattice-min (the union-find leader)
///   - `Primitive(_)`         => lattice-min (the term encoder's `:merge ordering-min`)
...
fn merge_mode_for_scalar(merge: &MergeFn) -> MergeMode {
    match merge {
        ...
        MergeFn::UnionId | MergeFn::Primitive(_, _) => MergeMode::Min,
...
impl Backend for EGraph {
    ...
    fn add_table(&mut self, config: FunctionConfig) -> FunctionId {
        ...
        let merge = if !has_output {
            ...
            merge_mode_for_scalar(&config.merge)
        };
