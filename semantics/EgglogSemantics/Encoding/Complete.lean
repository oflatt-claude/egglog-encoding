...
term stays. `rbTgtR_rowJoined` is the same clause at the satisfiability witness's state, where
`fn` carries the content and this one is vacuous. -/
...
theorem ncRule_builds : Actions.Builds ncRule.actions ncTgt.sig := by
  ...
  obtain rfl : f = "F" := by
    simpa [ncRule, Expr.fns, Expr.fnsList] using hf
  exact ⟨rfl, ncTgt_isCtor_F⟩

/-- **The two head clauses together, at a rule that really fires.** `ncRule_queriesEncodable`'s
counterpart: the pair `unionsJoined_fire_satisfiable` carries at `rbRule`, stated here at the
source/target pair `ncTgt_encRule_fired` runs the encoded rule at. -/
theorem ncRule_head_clauses :
    Actions.Scoped ncRule.actions (Query.bind ncRule.query (Env.dom ncPre.toDatabase.env)) ∧
      Actions.Builds ncRule.actions ncTgt.sig :=
  ⟨ncRule_headScoped, ncRule_builds⟩
...
its encoding leave, with a rule registered on each side.

`rbRule` is chosen so that the round *fires*. Its query reads the view row `rbProgram`'s own
build wrote — `@WView((A)) ↦ ((W (A)), @Fiat)` — and its head builds the same application, so
each side writes a row it already holds and `FDatabase.sameData` is reached in one round. That
is what makes the whole block, rebuild included, reduce in the kernel: `rbState2` puts three
terms in `FDatabase.valueTerms`, so the encoded query's three variables enumerate 27
candidates. -/

/-- **A rule whose encoding fires at the witness state.** Its head is its premise, so a round
adds nothing to either side; `?y` sits at `W`'s key column, which is what makes the encoded
query read it. -/
def rbRule : Rule where
  query := [.expr (.app "W" [.var "y"])]
  actions := [.expr (.app "W" [.var "y"])]
  ruleset := rbRuleset

/-- Its encoding, at the index and the fresh-variable counter the encoder's own numbering
gives a program's first rule. -/
def rbEncRule : Rule := (encodeRule 0 rbRule 0).1

/-- Its query: one view read, with the source variable in the key column and the read's two
generated columns. -/
theorem rbEncRule_query :
    rbEncRule.query = [Pattern.values [.var "@v0", .var "@v1"] (viewName "W") [.var "y"]] := rfl

/-- Its head: `encodeBuild`'s two `set`s for `(W y)`, the view entry under `@Fiat`. -/
theorem rbEncRule_actions :
    rbEncRule.actions
      = [Action.set (termName "W") [.var "y", .app "W" [.var "y"]] [],
         Action.set (viewName "W") [.var "y"] [.app "W" [.var "y"], fiatE]] := rfl

/-- `rbSig` with the rule's own proof head, which is what `ruleProofDecls` emits for a source
rule: `(constructor @Rule_0 (@Proof) @Proof)`, at the one proof column its encoded query
reads. -/
def rbSigR : Signature := Function.update rbSig (ruleName 0) (some (proofDecl 1))

/-- **The source**: `rbSrc` with `rbRule` registered. -/
def rbSrcR : Database := { rbSrc with env := rbEnv, rules := {rbRule} }

/-- **The target**: `rbState2` with the rule's proof head declared and the rule's encoding
registered. -/
def rbTgtR : FDatabase := { rbState2 with sig := rbSigR, rules := [rbEncRule] }

theorem rbSrcR_terms : rbSrcR.terms = rbSrc.terms := Database.terms_setEnvRules

theorem rbTgtR_sig : rbTgtR.sig = rbSigR := rfl

theorem rbSrcR_rules_mem {r : Rule} (hr : r ∈ rbSrcR.rules) : r = rbRule := hr

/-- **The source round**, as a relation: `CmdStep` needs no computation, and the merge phase
is neutral on a constructor signature. -/
theorem rbSrcR_cmdStep_run :
    CmdStep rbSrcR (.run rbRuleset) (RunRules rbRuleset rbSrcR) := ⟨_, rfl, .refl⟩

/-- The two ends of the environment clause: the target's is the one a top-level `let` left. -/
theorem rbTgtR_env : rbTgtR.env = rbSrcR.env := rfl

/-- `Database.WF` and `Signature.AllConstructors` are blind to the rules
(`Database.WF.setEnvRules`). -/
theorem rbSrcR_ctorState : rbSrcR.CtorState where
  wf := rbSrc_wf.setEnvRules (fun b hb => rbSrc_wf.envInTerms b hb)
  sig := rbSrc_ctorState.sig

/-- **The rule's proof head is the only difference between the two signatures**, and
`proofDecl` has no `:merge`, so every constructor `rbSig` declares `rbSigR` declares too. -/
theorem rbSigR_isCtor_of_rbSig {f : FnName} (h : rbSig.IsCtor f) : rbSigR.IsCtor f := by
  by_cases hf : f = ruleName 0
  · rw [hf]; decide
  · obtain ⟨d, hd, hm⟩ := h
    refine ⟨d, ?_, hm⟩
    rw [rbSigR, Function.update_of_ne hf]
    exact hd

/-- **Every source constructor is one the target's signature declares too**, which is the
clause `unionsFire_false` withdraws: `rbSrcSig` declares `A` and the *unary* `W`, and the
encoded signature makes both skolem constructors (`rbState2_sig_mono`). -/
theorem rbTgtR_sig_mono : ∀ f, rbSrcR.sig.IsCtor f → rbTgtR.sig.IsCtor f := by
  intro f hf
  rw [rbTgtR_sig]
  exact rbSigR_isCtor_of_rbSig (by rw [← rbState2_sig]; exact rbState2_sig_mono f hf)

/-- **And `@Fiat` is one**, which is what a build's view entry names as its proof. -/
theorem rbTgtR_isCtor_fiatName : rbTgtR.sig.IsCtor fiatName := by
  rw [rbTgtR_sig]
  exact rbSigR_isCtor_of_rbSig (by rw [← rbState2_sig]; exact rbState2_isCtor_fiatName)

/-- **The substitution the encoder flattens the rule through leaves it alone**: `rbRule`'s
query names `?y`, not the global `x`, so the rule the target holds is the encoding of the
rule the source holds. -/
...
/-- **The source rules' encodability, with content**: one pattern, one atom, `?y` at `W`'s
key column and no primitive among the names. `ncRule_queriesEncodable` is the same clause at
`ncRule`. -/
theorem rbSrcR_queriesEncodable : ∀ r ∈ rbSrcR.rules,
  ...
  · intro p hp
    obtain rfl : p = Pattern.expr (.app "W" [.var "y"]) := by simpa [rbRule] using hp
    trivial
  · intro v hv
    obtain rfl : v = "y" := by
...
theorem rbTgtR_rowColumnsValued : rbTgtR.RowColumnsValued := by
  change ∀ r ∈ rbTgtR.rows, ∀ t ∈ r.args ++ r.out, t ∈ rbTgtR.valueTerms
  decide

@[inherit_doc rbState2_noAtEnv]
theorem rbTgtR_noAtEnv : rbTgtR.NoAtEnv := rbState2_noAtEnv

/-- The extra declaration and the extra rule move no equation, so the two denotations contain
each other and every reading transports. -/
theorem rbTgtR_contains : rbState2.toDatabase.Contained rbTgtR.toDatabase := ⟨fun _ hp => hp⟩

@[inherit_doc rbTgtR_contains]
theorem rbTgtR_contained : rbTgtR.toDatabase.Contained rbState2.toDatabase := ⟨fun _ hp => hp⟩

/-- **Every source term has an entry reading**, at both of `rbSrcR`'s terms and at positive
arity in the second. -/
theorem rbTgtR_readsAt : ∀ t ∈ rbSrcR.terms, ∃ e, ViewRepr rbTgtR.toDatabase t e := by
  intro t ht
  rw [rbSrcR_terms] at ht
  obtain ⟨e, he⟩ := rbState2_unionsInv.readsAt t ht
  exact ⟨e, ViewRepr.mono rbTgtR_contains he⟩

/-- `rbProgram` asserts nothing but reflexive equations, so this clause is vacuous here;
`ncTgt_unionsJoined` is it at a source with a real equation. -/
theorem rbTgtR_joinedAt : rbTgtR.toDatabase.UnionsJoined rbSrcR :=
  fun a b hab hne => absurd (rbSrc_diag (a, b) hab) hne

/-- **Every source term has a row reading**, at both of `rbSrcR`'s terms and at positive arity
in the second. -/
theorem rbTgtR_exists_rowRepr : ∀ t ∈ rbSrcR.terms, ∃ r, RowRepr rbTgtR t r := by
  intro t ht
  rw [rbSrcR_terms, rbSrc_terms] at ht
...
/-- **The derived clause `unionsJoined_fire` reads its premise through.**

`fn` has content — the state holds two view rows and the reading really is pinned by them —
...
theorem rbTgtR_rowJoined : rbTgtR.RowJoined where
  fn := fun _ _ _ h₁ h₂ =>
    rowRepr_unique (fun _ _ _ _ _ _ hr₁ hr₂ => by
      have h := rbState2_rowsUnique _ hr₁ _ hr₂ rfl rfl
      exact (by simpa using h : _ ∧ _).1) h₁ h₂
  edge := fun x y pf _ _ _ _ hout _ _ _ _ =>
    absurd (Database.Out.mono rbTgtR_contained hout) (rbState2_no_out_uf x y pf)

set_option maxRecDepth 100000 in
/-- **Every row carries a `:merge` or has no output column.** The two `@fView` rows take the
first arm, with the `:merge` `encodePrelude` declared and the two output columns a view row
has, one of them at positive arity. -/
theorem rbTgtR_rowMerges :
    ∀ r ∈ rbTgtR.rows, (rbTgtR.sig.mergeOf r.fn).isSome = true ∨ r.out = [] := by decide

/-- **A live view row's name carries a `:merge`, at the witness state.** -/
theorem rbTgtR_mergeOf_of_row {f : FnName} {es : List Term} {e pf : Term}
    (hrow : (⟨viewName f, es, [e, pf]⟩ : Row) ∈ rbTgtR.rows) :
    (rbTgtR.sig.mergeOf (viewName f)).isSome = true :=
  mergeOf_of_row_of_outNonempty rbTgtR_rowMerges hrow

/-- **The source head is scoped where it runs**: `?y` is the query's own variable, and the
scope it is read against is the one the top-level `let` left. -/
theorem rbRule_headScoped :
    Actions.Scoped rbRule.actions (Query.bind rbRule.query (Env.dom rbSrcR.env)) := by
  simp [rbRule, Env.dom, Action.Scoped, Expr.IsApp, Expr.Scoped, Expr.vars,
    Expr.varsList, Query.bind, Query.vars, Pattern.vars]

@[inherit_doc rbRule_headScoped]
theorem rbSrcR_headsScoped : ∀ r ∈ rbSrcR.rules,
    Actions.Scoped r.actions (Query.bind r.query (Env.dom rbSrcR.env)) := by
  intro r hr
  obtain rfl := rbSrcR_rules_mem hr
  exact rbRule_headScoped

/-- **And it builds**, read at the *target's* signature, which is where the clause reads it:
the name the head applies is the skolem `encodePrelude` declared. -/
theorem rbRule_builds : Actions.Builds rbRule.actions rbTgtR.sig := by
  refine ⟨fun f hf => ?_, trivial⟩
  obtain rfl : f = "W" := by simpa [rbRule, Expr.fns, Expr.fnsList] using hf
  exact ⟨rfl, (by decide : rbSigR.IsCtor "W")⟩

@[inherit_doc rbRule_builds]
theorem rbSrcR_headsBuild : ∀ r ∈ rbSrcR.rules, Actions.Builds r.actions rbTgtR.sig := by
  intro r hr
  obtain rfl := rbSrcR_rules_mem hr
  exact rbRule_builds

/-- **A live row reads back as an entry**, at every row the witness state holds. -/
theorem rbTgtR_viewRepr_of_rowRepr {t r : Term} (h : RowRepr rbTgtR t r) :
    ViewRepr rbTgtR.toDatabase t r :=
  ViewRepr.of_rowRepr_of_rowTerms (d := rbTgtR) rbState2_rowColumns rbState2_rowEntries h

/-! #### And the round is not degenerate

The encoded rule matches, its head evaluates at the match, and the row it writes is one the
round's post-state records — the same three steps `ncTgt_encRule_fires` runs, at the state
whose whole block also reduces. -/

/-- `(A)`, the key `rbProgram`'s own build left the view row at. -/
def rbA : Term := .app "A" []

/-- `(W (A))`, the e-class column of that row and the term the rule's head rebuilds. -/
def rbWA : Term := .app "W" [rbA]

/-- **The id substitution.** The source rule fires at `?y := (A)`; the encoded one fires at
the same key, and at the row's own two output columns. -/
def rbIdSubst : Env := [("@v0", rbWA), ("@v1", .app fiatName []), ("y", rbA)]

theorem rbTgtR_freeVars : Query.freeVars rbEncRule.query [] = ["@v0", "@v1", "y"] := rfl

theorem rbTgtR_canon :
    Env.canon (Query.freeVars rbEncRule.query []) rbIdSubst = rbIdSubst := rfl

set_option maxRecDepth 100000 in
/-- **The encoded query matches at the id substitution.** -/
theorem rbTgtR_mem_matchQuery : rbIdSubst ∈ matchQuery rbTgtR rbEncRule.query := by
  rw [← rbTgtR_canon]
  refine mem_matchQuery_of_rows rbTgtR_rowColumnsValued (fun v hv => ?_) (fun v hv t ht => ?_)
    (fun p hp => ?_)
  · rw [rbTgtR_freeVars] at hv
    rcases (by simpa using hv : v = "@v0" ∨ v = "@v1" ∨ v = "y") with rfl | rfl | rfl <;> rfl
  · rw [rbTgtR_freeVars] at hv
    rcases (by simpa using hv : v = "@v0" ∨ v = "@v1" ∨ v = "y") with rfl | rfl | rfl <;>
      · obtain rfl : t = _ := Option.some.inj ht.symm
        decide
  · obtain rfl : p = Pattern.values [.var "@v0", .var "@v1"] (viewName "W") [.var "y"] := by
      simpa [rbEncRule_query] using hp
    exact ⟨_, _, _, [rbA], [rbWA, .app fiatName []], rfl, by decide, rfl, rfl,
      rbState2_row_wview⟩

/-- The head's key and skolem, evaluated at the id substitution. -/
theorem rbTgtR_evalList_head :
    Expr.evalList rbTgtR.sig [Expr.var "y", .app "W" [.var "y"]] (rbIdSubst ++ rbTgtR.env)
      = some [rbA, rbWA] := by
  have hy : Expr.eval rbTgtR.sig (.var "y") (rbIdSubst ++ rbTgtR.env) = some rbA := rfl
  simp only [Expr.evalList, hy, Option.bind_some, Option.map_some,
    Expr.eval_app_ctor (show Prim.ofName "W" = none from rfl)
      (show rbTgtR.sig.IsCtor "W" from (by decide : rbSigR.IsCtor "W"))]
  rfl

/-- The state the encoded rule's own firing returns: the two `set`s its head emitted, at the
ids the match bound — both of them rows the state already holds. -/
def rbFired : FDatabase :=
  { ((({ rbTgtR with env := rbIdSubst ++ rbTgtR.env } : FDatabase).addRow
        (termName "W") [rbA, rbWA] []).addRow
          (viewName "W") [rbA] [rbWA, .app fiatName []]) with
      env := rbTgtR.env, rules := rbTgtR.rules }

theorem rbFired_row :
    (⟨viewName "W", [rbA], [rbWA, .app fiatName []]⟩ : Row) ∈ rbFired.rows :=
  @mem_addRow_rows_self
    ((({ rbTgtR with env := rbIdSubst ++ rbTgtR.env } : FDatabase).addRow
      (termName "W") [rbA, rbWA] [])) (viewName "W") [rbA] [rbWA, .app fiatName []]

/-- **The block evaluates**, which is the half of a firing a valid substitution is not. -/
theorem rbTgtR_encRule_fired : Fired rbTgtR rbEncRule rbIdSubst rbFired := by
  change execLocalActions rbTgtR rbEncRule.actions rbIdSubst = some _
  rw [execLocalActions, rbEncRule_actions]
  simp only [execActions, Egglog.execAction, rbTgtR_evalList_head]
  rfl

/-- **The chain, end to end**: the row makes the encoded atom hold, the atom makes the
enumerator offer the id substitution, the block evaluates, and the head's row is one the
round's post-state records. -/
theorem rbTgtR_encRule_fires :
    (⟨viewName "W", [rbA], [rbWA, .app fiatName []]⟩ : Row)
      ∈ (execRunRules rbRuleset rbTgtR).rows :=
  mem_rows_execRunRules.mpr (Or.inr ⟨rbEncRule, List.mem_cons_self, rfl, rbIdSubst,
    rbTgtR_mem_matchQuery, rbFired, rbTgtR_encRule_fired, rbFired_row⟩)

set_option maxRecDepth 1000000 in
set_option maxHeartbeats 1000000 in
-- The kernel runs a round of rule firing here, and two merge phases after it; `unseal` is
-- what lets `closureF`'s `Finset` operations reduce at all.
unseal Egglog.closure in
/-- **And the whole block reduces.** The round of `rbRuleset`, its merge phase, and the
trailing `Cmd.saturate rebuildRuleset` — whose ruleset the state registers no rule under, so
the fixpoint is reached at once. The head writes what the state holds, so the post-state's
data is the pre-state's and `td'` is `td`. -/
theorem rbTgtR_run :
...
`rbTgtR_encRule_fired` the head evaluating at it, `rbTgtR_encRule_fires` the row the round's
...
**The clauses about names and rows are not degenerate.** `rbTgtR_sig_mono` carries both of
`rbSrcR`'s constructors, `A` and the *unary* `W`, onto the skolems `encodePrelude` declared —
which is exactly the clause `cxfTgt` withdraws — and `rbTgtR_isCtor_fiatName` is the head's own
proof. `rbTgtR_rowColumnsValued` is decided against the seven rows the state really holds, and
`rbTgtR_noAtEnv` against the environment a top-level `let` really left: `x`, not the empty
list. `rbTgtR_rowMerges`' two `@fView` rows take its **first** arm — the prelude's `:merge` and
the two output columns a view row has, one of them at positive arity — so
`rbTgtR_mergeOf_of_row` is the `:merge` carry with content and not the empty-output escape.
`rbTgtR_readsAt` and `rbTgtR_exists_rowRepr` read both of `rbSrcR`'s terms through entries and
...
`rbProgram` asserts nothing, so there is no `@UF` entry to follow (`rbSrc_diag`,
`rbState2_no_out_uf`). Both hold with content at `ncTgt` — `ncTgt_unionsJoined`, and
`ncTgt_rowJoined_edge`, where `(B)` reads to `(B)` and to `(A)` through entries while both read
to `(A)` alone through live **rows** — and `ncRule`'s encoding fires there too
(`ncTgt_encRule_fires`). What `ncTgt` cannot supply is *this* theorem's second conjunct.
`edge` holds there only because the rebuild re-keyed the displaced view row onto the leader,
and the proof terms that re-keying writes put **seven** terms in `FDatabase.valueTerms`, so the
encoded query's three variables enumerate 7³ candidates with one congruence closure each:
measured, the kernel passes 10 GB on that enumeration alone without finishing, against 3³ = 27
here. So the two clauses are exhibited at a state whose *round* the kernel cannot reduce and
the round is exhibited at a state with no `union`; neither clause is known to fail anywhere,
and no clause is weakened to keep them.

`difftest correspond`'s **LOST** column — `Cong src a b` without `SameClass tgt a b`, swept
with the diagonal included over the 87 in-domain cases, rules and runs among them — is 0,
which is the `reads` clause measured: every source term has *some* id in the target. Asking
for that id to be the term itself is the stronger claim the counterexample refutes
(`Database.ReadsSelf`, `ncTgt_not_readsSelf`), and this clause does not ask it. -/
theorem unionsJoined_fire_satisfiable :
    CmdStep rbSrcR (.run rbRuleset) (RunRules rbRuleset rbSrcR) ∧
      rbTgtR.execProgramM [Cmd.run rbRuleset, Cmd.saturate rebuildRuleset] = some rbTgtR ∧
      rbTgtR.env = rbSrcR.env ∧ rbSrcR.CtorState ∧
      (∀ f, rbSrcR.sig.IsCtor f → rbTgtR.sig.IsCtor f) ∧
      ...
      (∀ t ∈ rbSrcR.terms, ∃ e, ViewRepr rbTgtR.toDatabase t e) ∧
      rbTgtR.toDatabase.UnionsJoined rbSrcR ∧
      (∀ t ∈ rbSrcR.terms, ∃ r, RowRepr rbTgtR t r) ∧ rbTgtR.RowJoined ∧
      (∀ (f : FnName) (es : List Term) (e pf : Term),
        (⟨viewName f, es, [e, pf]⟩ : Row) ∈ rbTgtR.rows →
          (rbTgtR.sig.mergeOf (viewName f)).isSome = true) ∧
      (∀ r ∈ rbSrcR.rules,
  ...
  ⟨rbSrcR_cmdStep_run, rbTgtR_run, rbTgtR_env, rbSrcR_ctorState, rbTgtR_sig_mono,
    ...
    rbTgtR_rowColumnsValued, rbTgtR_noAtEnv, rbTgtR_readsAt, rbTgtR_joinedAt,
...
/-! #### What the `.eq` atom's membership is short of

`PatternRowRead.eq` carries `i ∈ d.terms`, and it has to: `matchQuery` decides an emitted `.eq`
atom against `FDatabase.closureF`, whose candidate universe is `terms`, so an id outside the
list is one no comparison holds at. The two lemmas below are the halves of that obligation that
are **theorems**, and what is left of it is exactly the third case.

**The unrestricted reading of it is false**, and by design rather than by defect:
`litBuild_not_litsHeld` is an in-domain program whose source holds `5` and whose encoded run
holds no term at all, because `encodeBuild` emits no action for a bare leaf and `ViewRepr.lit`
therefore carries no membership premise. So `Term.lit l ∈ sd.terms → Term.lit l ∈ td.terms` is
not a clause `Egglog.RowMech` can carry, and step 1's residue is the **bare-literal side of a
`.eq`** and not an arbitrary literal: `Pattern.Grounded` excludes that shape from the program's
text, `Pattern.GroundedAt` is what survives `Rule.resolveGlobals`, and what the surviving case
needs is that a literal-valued *global*'s value is a term the target holds — which the encoded
`let` records (`encodeAction` emits `.letBind v (.lit l)` and `FDatabase.addTerm` inserts it),
and which is therefore a clause about `sd.env` rather than about `sd.terms`. -/

/-- **An id a `RowRepr` gives for an application is a term the target holds.**
`FDatabase.RowColumnsValued` — already a clause of `Egglog.UnionsFire` — puts the row's value
column in `FDatabase.valueTerms`, hence in `terms`. -/
theorem mem_terms_of_rowRepr_app {td : FDatabase} (hcv : td.RowColumnsValued)
    {f : FnName} {as : List Term} {r : Term} (h : RowRepr td (.app f as) r) : r ∈ td.terms := by
  cases h with
  | app _ hrow =>
      exact FDatabase.mem_terms_of_mem_valueTerms
        (hcv _ hrow r (List.mem_append_right _ List.mem_cons_self))

/-- **And an id a `RowRead` gives is one, away from a bare literal.** The `var` case is the
target reading's own value — which `mem_matchQuery_encodeQuery` already asks to be a
`FDatabase.valueTerms` member, so nothing new is spent — and the `app` case is the row's. -/
theorem mem_terms_of_rowRead {sig : Signature} {td : FDatabase} {ρs ρt : Env}
    (hcv : td.RowColumnsValued)
    (hvt : ∀ (v : Var) (i : Term), Env.lookup v ρt = some i → i ∈ td.terms)
    {e : Expr} {t i : Term} (h : RowRead sig td ρs ρt e t i) (hne : ∀ l, e ≠ Expr.lit l) :
    i ∈ td.terms := by
  cases h with
  | lit => exact absurd rfl (hne _)
  | var _ hi => exact hvt _ _ hi
  | app _ _ _ _ hrow =>
      exact FDatabase.mem_terms_of_mem_valueTerms
        (hcv _ hrow i (List.mem_append_right _ List.mem_cons_self))

/-- **So the third case is vacuous on the program's text.** `Pattern.Grounded` asks one side of
an equality not to be a bare literal, which is exactly the shape left over — the surviving
instance is the one `Rule.resolveGlobals` creates, where a literal-valued global's value was
written into *both* sides and the source's own environment binds it. -/
theorem Pattern.Grounded.eqLit {d : FDatabase} : ∀ {p : Pattern}, p.Grounded →
    ∀ l : Lit, p = Pattern.eq (Expr.lit l) (Expr.lit l) → Term.lit l ∈ d.terms
  | .eq _ _, h, l, hp => by
      rw [Pattern.eq.injEq] at hp
      rcases h with h | h
      · exact absurd hp.1 (h l)
      · exact absurd hp.2 (h l)
...
**The `.eq` atom's membership is paid from the reading's own shape**, and not from the clause
that used to stand here: "every id the reading gives is a term the target holds" is **false**
(`litBuild_not_litsHeld`), so the obligation is split by the case the reading is in —
`FDatabase.RowColumnsValued` for a row, the target reading's own value for a variable, and
`hgl` for the one case left, both sides a bare literal of the same value. `hgl` is vacuous on
the program's text (`Pattern.Grounded.eqLit`); what survives it is the pattern
`Rule.resolveGlobals` creates out of a literal-valued global. -/
theorem patternRowRead_of_matches {sd : Database} {td : FDatabase} {σ ρt : Env}
    ...
    (hgl : ∀ l : Lit, p = Pattern.eq (Expr.lit l) (Expr.lit l) → Term.lit l ∈ td.terms)
  ...
  | eq hw hev₁ hev₂ hcw hc12 =>
      ...
      have k₁ := rowRead_of_rowRepr hjoin.fn hmg hvar _
        (fun g hg => hprim g (List.mem_union_iff.mpr (Or.inl hg))) hev₁ hr₁
      have k₂ := rowRead_of_rowRepr hjoin.fn hmg hvar _
        (fun g hg => hprim g (List.mem_union_iff.mpr (Or.inr hg))) hev₂ hr₂
      refine .eq k₁ k₂ ?_
      -- the id is a target term unless *both* sides are bare literals, which
      -- `Pattern.Grounded` excludes from the program's text
      cases k₁ with
      | var _ hi => exact hvt _ _ hi
      | app _ _ _ _ hrow =>
          exact FDatabase.mem_terms_of_mem_valueTerms
            (hcv _ hrow _ (List.mem_append_right _ List.mem_cons_self))
      | lit =>
          cases k₂ with
          | var _ hi => exact hvt _ _ hi
          | app _ _ _ _ hrow =>
...
below is the clause set `Egglog.UnionsFire` used to have, `unionsFire_of_weak` says it is the
stronger claim, and `cxfTgt_not_sigMono` and `cxpSrc_not_queriesEncodable` name the clause each
witness violates. The reading below is of that old statement.

`UnionsFireWeak` quantifies over an arbitrary `td` under ten hypotheses, and **not one of
...
rather than at the target's signature. All four are now clauses of `Egglog.UnionsFire`, plus
the `@Rule_i` one the same reading gives at a `union` head; `Egglog.RowMech` discharges four of
them and `Egglog.RuleNameMech` the fifth. -/

/-- **The clause set the two refutations below kill**: `Egglog.UnionsFire` with its clauses
...
repaired statement — which is the point: `cxfTgt_not_sigMono` and
`cxpSrc_not_queriesEncodable` are the clauses the two witnesses violate. -/
def UnionsFireWeak : Prop :=
  ∀ {R : RulesetName} {c : Cmd} {sd sd' : Database} {td td' : FDatabase},
    ...
    (∀ r ∈ sd.rules, ∃ (G : List (Var × Expr)) (i n : Nat),
      (encodeRule i (r.substGlobals G) n).1 ∈ td.rules) →
...
/-- **`Egglog.UnionsFireWeak` is false.** Ten hypotheses, all of them satisfied, and the
`reads` half of the conclusion failing at `(H)` — the term the source's own firing built and
the target's could not, because nothing in that clause set makes the encoded head evaluate.

Not a refutation of `encode`: the missing fact is `FDatabase.EncBase`'s `sig` clause, which
every state an encoded run reaches carries (`encReached_encBase`). It was a refutation of the
residue **as it was then stated**, and the clause — with the three the docstring above names
beside it — is now among `Egglog.UnionsFire`'s own, discharged by `Egglog.encStep_rowMech` the
way `Egglog.RowMech`'s row clauses are. `cxfTgt_not_sigMono` is this witness failing it. -/
theorem unionsFire_false : ¬ UnionsFireWeak := by
  intro h
  obtain ⟨-, hreads⟩ :=
...
/-! ##### And the clause that kills it

`Egglog.UnionsFire` asks the target's signature to declare every constructor the source's
does. `cxfTgt` is the encoder's own signature with `H` withdrawn, so it does not — and that is
the whole of what the witness violates: its query is empty, so the source-rules clause holds
at it outright, and its rows and environment are empty, so the two the enumerator reads hold
too. -/

/-- **The clause `cxfTgt` violates.** `cxfSrcSig` makes `H` a constructor and `cxfTgt.sig`
withholds it, which is exactly why the encoded head does not evaluate. -/
theorem cxfTgt_not_sigMono : ¬ ∀ f, cxfSrc.sig.IsCtor f → cxfTgt.sig.IsCtor f := by
  intro h
  obtain ⟨dc, hdc, -⟩ := h "H" ⟨{ arity := 0, outArity := 1, merge := none }, rfl, rfl⟩
  exact absurd hdc (by simp [cxfTgt])
...
theorem cxfSrc_queriesEncodable : ∀ r ∈ cxfSrc.rules,
  ...
  exact ⟨⟨by simp [cxfRule], by intro v hv; exact absurd hv (by simp [cxfRule])⟩,
    by intro p hp; exact absurd hp (by simp [cxfRule])⟩

set_option maxRecDepth 10000 in
/-- **As do the other three**: `@Fiat` and `@Rule_0` are declared — only `H` was withdrawn —
and the target holds no row and binds nothing. -/
theorem cxfTgt_other_clauses :
    cxfTgt.sig.IsCtor fiatName ∧ cxfTgt.sig.IsCtor (ruleName 0) ∧
      cxfTgt.RowColumnsValued ∧ cxfTgt.NoAtEnv :=
  ⟨by decide, by decide,
    fun r hr => absurd hr (by simp [cxfTgt]),
    fun b hb => absurd hb (by simp [cxfTgt])⟩
...
/-- **`Egglog.UnionsFireWeak` is false a second time, at a target the encoder's own signature
and provenance would supply.** So the repair was not one derived clause but two, on opposite
sides of the correspondence: the target's signature, which `FDatabase.EncBase` has, and the
source rules' encodability, which `Program.EncodeDomain.noPrim` and `queryEncodable` have and
which no target-side clause can reach — `queriesIn_of_prefixStep` is that one carried by the
source run instead, since `Egglog.UnionsFire` is given no program.
`cxpSrc_not_queriesEncodable` is this witness failing it, and `cxpTgt_name_clauses` is every
clause about names holding at its target. -/
theorem unionsFire_false_encodeSig : ¬ UnionsFireWeak := by
  intro h
  obtain ⟨-, hreads⟩ :=
...
/-! ##### And the clause that kills the second one

The target here is the encoder's own signature on the nose, so every clause about names holds
at it; what fails is the clause about the **source's** rules, and it fails at the same name
`Program.EncodeDomain.noPrim` rejects the program for. -/

/-- **The clause `cxpSrc` violates.** `Pattern.ctors` records `("min", 2)` and `min` is a
primitive, so the source query evaluates through `Prim.apply` where the encoded one reads a
table nothing writes. -/
theorem cxpSrc_not_queriesEncodable : ¬ ∀ r ∈ cxpSrc.rules, (Cmd.rule r).QueryEncodable ∧
    ∀ p ∈ r.query, ∀ fk ∈ p.ctors, Prim.ofName fk.1 = none := by
  intro h
  have := (h cxpRule rfl).2 (Pattern.expr (.app "min" [.var "x", .var "x"]))
    (by simp [cxpRule]) ("min", 2) (by decide)
  exact absurd this (by decide)

set_option maxRecDepth 10000 in
/-- **And every clause about names holds at `cxpTgt`**, which is what says the repair is not
one clause but two on opposite sides: `cxpSrc`'s one constructor is `H` and `encodeSig` makes
it a skolem, `@Fiat` and `@Rule_0` are the prelude's own, and the target holds no row and
binds nothing. -/
theorem cxpTgt_name_clauses :
    (∀ f, cxpSrc.sig.IsCtor f → cxpTgt.sig.IsCtor f) ∧
      cxpTgt.sig.IsCtor fiatName ∧ cxpTgt.sig.IsCtor (ruleName 0) ∧
      cxpTgt.RowColumnsValued ∧ cxpTgt.NoAtEnv := by
  refine ⟨?_, by decide, by decide,
    fun r hr => absurd hr (by simp [cxpTgt]),
    fun b hb => absurd hb (by simp [cxpTgt])⟩
  intro f hf
  by_cases hH : f = "H"
  · rw [hH]; exact (by decide : cxpTgt.sig.IsCtor "H")
  · obtain ⟨dc, hdc, -⟩ := hf
    have hnone : cxfSrcSig f = none := by rw [cxfSrcSig, if_neg hH]
    have hdc' : cxfSrcSig f = some dc := hdc
    rw [hnone] at hdc'
...
The witness reuses `rbState2`, the state `unionsJoined_fire_satisfiable`'s own `rbTgtR` is
built from, so the arithmetic is the same and the *only* difference is the rule: `rbSrc` with
one rule added, and the target with that rule's encoding through a bad `G`. Everything below is
compiled. -/
...
/-- **The target**: `rbState2` with the encoded rule installed and `@Rule_0` declared, which is
how `unionsJoined_fire_satisfiable`'s own `rbTgtR` is built too. -/
...
/-! ### The three clauses about names, `@Rule_i` among them

`Egglog.UnionsFire`'s head has to *evaluate*, and `Expr.eval` is `none` at a name the
signature does not make a constructor. Three families of name reach it: the source
constructors a build applies, `@Fiat`, and the `@Rule_i` a `union` head names as its
justification. The first two are one lemma each; the third has to be threaded, because
`ruleProofDecls` declares `@Rule_i` only at the indices the encoder's own numbering reaches
and `encodeRule` is a function of an arbitrary one. -/

/-- **A source constructor is a constructor of `encodeSig P`.** `encodePrelude` emits
`.decl f (skolemDecl k)` per entry of `Program.ctors`, and a source name is `@`-free, so none
of the prelude's `:merge`-carrying declarations can be at it. -/
theorem encodeSig_isCtor_of_mem_ctors {P : Program} (hdom : P.EncodeDomain)
    {fk : FnName × Nat} (h : fk ∈ P.ctors) : (encodeSig P).IsCtor fk.1 := by
  have hat := noAt_of_mem_ctors hdom h
  have huf : fk.1 ≠ ufName := fun hc =>
    hat (by rw [hc]; exact (by decide +kernel : "@".isPrefixOf ufName = true))
  have hv : ∀ g, fk.1 ≠ viewName g := fun g hc =>
    hat (by rw [hc]; exact isPrefixOf_at_viewName g)
  have ht : ∀ g, fk.1 ≠ termName g := fun g hc =>
    hat (by rw [hc]; exact isPrefixOf_at_termName g)
  have hmem : Cmd.decl fk.1 (skolemDecl fk.2) ∈ encodePrelude P := by
    rw [encodePrelude]
    refine List.mem_append_left _ (List.mem_append_right _ (List.mem_cons_of_mem _
      (List.mem_flatMap.mpr ⟨fk, h, ?_⟩)))
    exact List.mem_cons_self
  rw [encodeSig]
  exact isCtor_foldl_sigBind
    (fun _ hdc' => merge_none_of_mem_encodePrelude huf hv ht hdc')
    (Or.inr ⟨skolemDecl fk.2, hmem⟩)

/-- **A term relation is never a rule's proof head**, by the `'m'` of its suffix. Mirrors
`viewName_ne_ruleName`. -/
theorem termName_ne_ruleName {f : FnName} {i : Nat} : termName f ≠ ruleName i := by
  intro h
  have hw : 'm' ∈ (termName f).toList := by
    rw [termName, String.toList_append]
    exact List.mem_append_right _ (by decide)
  ...
  · exact absurd h1 (by decide)
  · have hd := Nat.isDigit_of_mem_toDigits (b := 10) (by decide) (by decide)
      (show 'm' ∈ Nat.toDigits 10 i by rw [Nat.toString_eq_repr, Nat.toList_repr] at h1; exact h1)
    simp at hd

/-- **`@Rule_i` is a constructor of `encodeSig P`** wherever the vocabulary declares it. -/
theorem encodeSig_isCtor_ruleName {P : Program} {i : Nat} {dc : FnDecl}
    (h : Cmd.decl (ruleName i) dc ∈ proofDecls P) : (encodeSig P).IsCtor (ruleName i) :=
  encodeSig_isCtor_of_mem_proofDecls h (fun hc => ufName_ne_ruleName hc.symm)
    (fun _ h' => viewName_ne_ruleName h'.symm) (fun _ h' => termName_ne_ruleName h'.symm)

/-- **The numbering, read off `ruleProofDecls` itself**: it walks the program with the same
counter `encodeCmds` does and emits one `@Rule_i` declaration per rule command. -/
theorem ruleNamesDeclared_of_mem {P : Program} {sg : Signature} :
    ∀ (G : List (Var × Expr)) (p : Program) (i : Nat),
      (∀ (j : Nat) (dc : FnDecl), Cmd.decl (ruleName j) dc ∈ ruleProofDecls P G p i →
        sg.IsCtor (ruleName j)) →
      Program.RuleNamesDeclared sg p i
  | _, [], _, _ => trivial
  | G, c :: cs, i, h => by
      refine ⟨?_, ruleNamesDeclared_of_mem (c.globalBind P G) cs (c.ruleStep i)
        (fun j dc hj => h j dc (by rw [ruleProofDecls]; exact List.mem_append_right _ hj))⟩
      intro r hr
      subst hr
      refine h i (proofDecl (ruleProofArity (r.substGlobals G))) ?_
      rw [ruleProofDecls]
      exact List.mem_append_left _ (by rw [Cmd.proofDeclOf]; exact List.mem_cons_self)

/-- **Every `@Rule_i` the encoder's numbering of `P` applies is declared by its prelude.** -/
theorem ruleNamesDeclared_encodeSig {P : Program} :
    Program.RuleNamesDeclared (encodeSig P) P 0 :=
  ruleNamesDeclared_of_mem (P := P) [] P 0 fun _ _ hj =>
    encodeSig_isCtor_ruleName (by rw [proofDecls]; exact List.mem_append_right _ hj)
...
theorem cmdStep_queriesIn {P : Program} (hdom : P.EncodeDomain) {db db' : Database}
  ...
  | action a =>
      intro r hr
      exact h r (by rw [evalAction_rules (cmdStep_action_eq hsig hstep)] at hr; exact hr)
  | rule r =>
      obtain ⟨-, -, hrules⟩ := cmdStep_rule_fields hsig hstep
      intro s hs
      rw [hrules] at hs
  ...
  | decl f dc =>
      obtain ⟨-, -, hrules⟩ := cmdStep_decl_fields hsig (hdom.ctorsOnly _ hc) hstep
      intro r hr
      exact h r (by rw [hrules] at hr; exact hr)

/-- **And every constructor the source signature declares is one the program's census
names**, since only `Cmd.decl` writes the signature and `Cmd.ctors` records its pair. -/
def Database.SigIn (db : Database) (P : Program) : Prop :=
  ∀ f, db.sig.IsCtor f → ∃ k, (f, k) ∈ P.ctors

@[inherit_doc cmdStep_queriesIn]
theorem cmdStep_sigIn {P : Program} {db db' : Database} (h : db.SigIn P) {c : Cmd} (hc : c ∈ P)
    (hstep : CmdStep db c db') : db'.SigIn P := by
  intro f hf
  rw [CmdStep.sig hstep] at hf
  cases c with
  | decl g dc =>
      by_cases hg : f = g
      · subst hg
        exact ⟨dc.arity, mem_program_ctors hc (by rw [Cmd.ctors]; exact List.mem_cons_self)⟩
      · refine h f ?_
        obtain ⟨e, he, hm⟩ := hf
        exact ⟨e, by rw [Cmd.sigBind, Function.update_of_ne hg] at he; exact he, hm⟩
  | _ => exact h f hf
...
theorem queriesIn_of_prefixStep {P : Program} (hdom : P.EncodeDomain) {p : Program}
    (hsub : ∀ c ∈ p, c ∈ P) {sd : Database} (hstep : ProgramStep Database.empty p sd) :
    sd.QueriesIn :=
...
@[inherit_doc queriesIn_sigIn_of_prefixStep]
theorem sigIn_of_prefixStep {P : Program} (hdom : P.EncodeDomain) {p : Program}
    (hsub : ∀ c ∈ p, c ∈ P) {sd : Database} (hstep : ProgramStep Database.empty p sd) :
    sd.SigIn P :=
  (queriesIn_sigIn_of_prefixStep hdom hsub Database.CtorState.empty
    Database.empty_termsBuilds
...
already had is about the head's *query*. `Database.HeadsBuild` with `headsBuild_of_programStep`
is the second of them at `sd.sig`, and `Actions.Builds.mono_sig` reads it at `td.sig` across
the signature clause. `Database.HeadsScoped` is the first, and what it costs is that a
*stored* rule's query is `Rule.resolveGlobals`'d: a variable the resolution removed is a
global and the environment binds it, so the scope the head is checked in only widens. -/

/-- **Resolving the globals into an expression removes only variables the environment binds.**
-/
theorem Expr.mem_vars_resolveGlobals (σ : Env) (e : Expr) :
    ∀ v ∈ e.vars, v ∈ (Expr.resolveGlobals σ e).vars ∨ v ∈ Env.dom σ := by
  induction e using Expr.rec
    (motive_2 := fun es => ∀ v ∈ Expr.varsList es,
      v ∈ Expr.varsList (Expr.resolveGlobalsList σ es) ∨ v ∈ Env.dom σ) with
  | lit l => intro v hv; exact absurd hv (by simp [Expr.vars])
  | var w =>
      intro v hv
      obtain rfl : v = w := by simpa [Expr.vars] using hv
      rcases hlk : Env.lookup v σ with _ | t
      · refine Or.inl ?_
        rw [Expr.resolveGlobals_var_none hlk, Expr.vars]
        exact List.mem_cons_self
      · exact Or.inr (Env.lookup_isSome_iff_mem_dom.mp (by rw [hlk]; rfl))
  | app f args ih =>
      intro v hv
      rw [Expr.vars] at hv
      rw [Expr.resolveGlobals_app, Expr.vars]
      exact ih v hv
  | nil => rename_i v hv; exact absurd hv (by simp [Expr.varsList])
  | cons e es ihe ihes =>
      rename_i v hv
      rw [Expr.varsList, List.mem_union_iff] at hv
      rw [Expr.resolveGlobalsList_cons, Expr.varsList]
      rcases hv with hv | hv
      · exact (ihe v hv).imp (fun h => List.mem_union_iff.mpr (Or.inl h)) id
      · exact (ihes v hv).imp (fun h => List.mem_union_iff.mpr (Or.inr h)) id

@[inherit_doc Expr.mem_vars_resolveGlobals]
theorem Expr.mem_varsList_resolveGlobalsList (σ : Env) : ∀ (es : List Expr),
    ∀ v ∈ Expr.varsList es,
      v ∈ Expr.varsList (Expr.resolveGlobalsList σ es) ∨ v ∈ Env.dom σ
  | [], v, hv => absurd hv (by simp [Expr.varsList])
  | e :: es, v, hv => by
      rw [Expr.varsList, List.mem_union_iff] at hv
      rw [Expr.resolveGlobalsList_cons, Expr.varsList]
      rcases hv with hv | hv
      · exact (Expr.mem_vars_resolveGlobals σ e v hv).imp
          (fun h => List.mem_union_iff.mpr (Or.inl h)) id
      · exact (Expr.mem_varsList_resolveGlobalsList σ es v hv).imp
          (fun h => List.mem_union_iff.mpr (Or.inr h)) id

@[inherit_doc Expr.mem_vars_resolveGlobals]
theorem Pattern.mem_vars_resolveGlobals (σ : Env) : ∀ (p : Pattern),
    ∀ v ∈ p.vars, v ∈ (Pattern.resolveGlobals σ p).vars ∨ v ∈ Env.dom σ
  | .expr e, v, hv => Expr.mem_vars_resolveGlobals σ e v hv
  | .eq e₁ e₂, v, hv => by
      rw [Pattern.vars, List.mem_union_iff] at hv
      rw [Pattern.resolveGlobals, Pattern.vars]
      rcases hv with hv | hv
      · exact (Expr.mem_vars_resolveGlobals σ e₁ v hv).imp
          (fun h => List.mem_union_iff.mpr (Or.inl h)) id
      · exact (Expr.mem_vars_resolveGlobals σ e₂ v hv).imp
          (fun h => List.mem_union_iff.mpr (Or.inr h)) id
  | .values vs f as, v, hv => by
      rw [Pattern.vars, List.mem_union_iff] at hv
      rw [Pattern.resolveGlobals, Pattern.vars]
      rcases hv with hv | hv
      · exact (Expr.mem_varsList_resolveGlobalsList σ vs v hv).imp
          (fun h => List.mem_union_iff.mpr (Or.inl h)) id
      · exact (Expr.mem_varsList_resolveGlobalsList σ as v hv).imp
          (fun h => List.mem_union_iff.mpr (Or.inr h)) id

@[inherit_doc Expr.mem_vars_resolveGlobals]
theorem Query.mem_vars_resolveGlobals (σ : Env) {q : Query} {v : Var}
    (hv : v ∈ Query.vars q) :
    v ∈ Query.vars (Query.resolveGlobals σ q) ∨ v ∈ Env.dom σ := by
  obtain ⟨p, hp, hvp⟩ := Query.mem_vars.mp hv
  refine (Pattern.mem_vars_resolveGlobals σ p v hvp).imp (fun h => ?_) id
  exact Query.mem_vars.mpr ⟨Pattern.resolveGlobals σ p, List.mem_map_of_mem hp, h⟩

/-- **Every rule the state holds has a head the rule's own query and the globals scope.**
`Rule.HeadScoped` at each, which is what `evalLocalActions_isSome_of_builds` asks. -/
def Database.HeadsScoped (db : Database) : Prop := ∀ r ∈ db.rules, r.HeadScoped db

theorem Database.empty_headsScoped : Database.empty.HeadsScoped := by
  intro r hr; exact absurd hr (by simp [Database.empty])

/-- The scope only widens as the environment grows. -/
theorem Rule.HeadScoped.mono_env {db db' : Database}
    (hsub : ∀ v ∈ Env.dom db.env, v ∈ Env.dom db'.env) {r : Rule} (h : r.HeadScoped db) :
    r.HeadScoped db' :=
  Actions.Scoped.mono r.actions
    (fun v hv => by
      rw [Query.bind, List.mem_union_iff] at hv ⊢
      exact hv.imp (hsub v) id) h

/-- **A registered rule's head is scoped where the state stores it.** The rule the state
holds is the declared one with its query resolved, and a variable the resolution removed is
one the environment binds. -/
theorem Rule.headScoped_resolveGlobals {db : Database} {s : Rule}
    (h : Actions.Scoped s.actions (Query.vars s.query)) :
    (s.resolveGlobals db.env).HeadScoped db := by
  refine Actions.Scoped.mono s.actions (fun v hv => ?_) h
  rw [Rule.resolveGlobals_query, Query.bind, List.mem_union_iff]
  exact (Query.mem_vars_resolveGlobals db.env hv).symm

/-- **A top-level action only extends the environment.** -/
theorem evalAction_env_dom {db db' : Database} {a : Action}
    (h : evalAction db a = some db') : ∀ v ∈ Env.dom db.env, v ∈ Env.dom db'.env := by
  rcases evalAction_eq_some h with ⟨_, _, -, -, rfl⟩ | ⟨w, _, t, -, -, rfl⟩ |
    ⟨_, _, _, _, -, -, -, -, rfl⟩ | ⟨_, _, _, _, _, -, -, -, rfl⟩
  · exact fun v hv => hv
  · intro v hv
    change v ∈ Env.dom ((w, t) :: db.env)
    rw [Env.dom, List.map_cons]
    exact List.mem_cons_of_mem _ (by rw [Env.dom] at hv; exact hv)
  · exact fun v hv => hv
  · exact fun v hv => hv

/-- **One command keeps it**: only `Cmd.rule` extends `rules`, only `Cmd.action` moves the
environment, and neither undoes the other's clause. -/
theorem cmdStep_headsScoped {P : Program} (hhs : P.HeadsScoped) {db db' : Database}
    (hsig : db.sig.AllConstructors) (h : db.HeadsScoped) {c : Cmd}
    (hc : c ∈ P) (hdecl : c.CtorDecl) (hstep : CmdStep db c db') : db'.HeadsScoped := by
  cases c with
  | action a =>
      have hv : evalAction db a = some db' := cmdStep_action_eq hsig hstep
      intro r hr
      exact (h r (by rw [evalAction_rules hv] at hr; exact hr)).mono_env (evalAction_env_dom hv)
  | rule s =>
      obtain ⟨-, henv, hrules⟩ := cmdStep_rule_fields hsig hstep
      intro r hr
      rw [hrules] at hr
      rcases Set.mem_insert_iff.mp hr with rfl | hr'
      · exact (Rule.headScoped_resolveGlobals (hhs _ hc)).mono_env
          (fun v hv => by rw [henv]; exact hv)
      · exact (h r hr').mono_env (fun v hv => by rw [henv]; exact hv)
  | run R =>
      intro r hr
      exact (h r (by rw [cmdStep_rules_of_run hstep] at hr; exact hr)).mono_env
        (fun v hv => by rw [cmdStep_env_of_run hstep]; exact hv)
  | saturate R =>
      intro r hr
      exact (h r (by rw [cmdStep_rules_of_saturate hstep] at hr; exact hr)).mono_env
        (fun v hv => by rw [cmdStep_env_of_saturate hstep]; exact hv)
  | decl f dc =>
      obtain ⟨-, henv, hrules⟩ := cmdStep_decl_fields hsig hdecl hstep
      intro r hr
      exact (h r (by rw [hrules] at hr; exact hr)).mono_env
        (fun v hv => by rw [henv]; exact hv)

@[inherit_doc cmdStep_headsScoped]
theorem headsScoped_of_prefixStep {P : Program} (hdom : P.EncodeDomain) :
    ∀ {p : Program} {db db' : Database}, (∀ c ∈ p, c ∈ P) → db.CtorState →
      db.HeadsScoped → ProgramStep db p db' → db'.HeadsScoped := by
  intro p
  induction p with
  | nil =>
      intro db db' _ _ hs hstep
      obtain rfl := hstep.nil_inv
      exact hs
  | cons c cs ih =>
      intro db db' hsub hcs hs hstep
      obtain ⟨db₁, hstep₁, hrest⟩ := hstep.cons_inv
      exact ih (fun c' hc' => hsub c' (List.mem_cons_of_mem c hc'))
        (hstep₁.ctorState hcs (hdom.ctorsOnly c (hsub c List.mem_cons_self)))
        (cmdStep_headsScoped hdom.headsScoped hcs.sig hs (hsub c List.mem_cons_self)
          (hdom.ctorsOnly c (hsub c List.mem_cons_self)) hstep₁) hrest
...
`sigIn_of_prefixStep`) and `@Fiat` (`encodeSig_isCtor_fiatName`), so the encoded head
evaluates; the source rules' queries are ones the flattening handles
(`queriesIn_of_prefixStep`), which is what `Program.EncodeDomain.queryEncodable` and `noPrim`
pay and what `UnionsFire` cannot ask of a program it is not given;
`encReached_rowColumnsValued` is where `matchQuery` draws its candidates from, and
`FDatabase.EncBase.noAtEnv` is the environment binding no generated variable.
...
witness state, and all but the two that need a source `union` non-vacuously. -/
theorem encStep_rowMech {P : Program} (hdom : P.EncodeDomain)
  ...
  fun h => ⟨fun _ _ hv => encStep_exists_rowRepr hdom hnodup h hv,
    ...
    headsScoped_of_prefixStep hdom h.mem Database.CtorState.empty
      Database.empty_headsScoped h.src,
    fun r hr =>
      Actions.Builds.mono_sig
        (fun f hf => by
          obtain ⟨k, hk⟩ := sigIn_of_prefixStep hdom h.mem h.src f hf
          rw [(encReached_encBase hdom h.reached).sig]
          exact encodeSig_isCtor_of_mem_ctors (fk := (f, k)) hdom hk)
